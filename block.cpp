...
// README.txt http://www.zedwood.com/article/cpp-sha256-function
...
//Node::Node(uint8_t private_key, uint8_t public_key, string balance, vector<block *> BLOCK_CHAIN)
//{
//}
...
// Merkle tree ở đây là 1 binary-tree bởi vì cứ mỗi 2 node là 2 transaction dc tạo ra(1 cho ng gửi và 1 cho ng nhận), tránh dc douple-spend, cứ như thế mà merkle tree dc tạo ra
std::string block::Hash_A_Block(int index, std::vector<tranx> transactions, std::string previous_hash, std::string timestamp, long long difficulty, long long nonce) {
	std::string hash_of_merkle_tree = "NONCE";

	if (transactions.empty() == false) {
		hash_of_merkle_tree = "";
		for (int i = 0; i < transactions.size(); i++) {
			//hash_of_merkel_tree += std::to_string(transactions[i]);
		}
	}
	...
	// Difficulty < len(sha(256))
	auto start = std::chrono::system_clock::now();
	while (true)
	{
		bool flag = true;
		for (long long i = 1; i <= difficulty; i++) {
			hash_sha = sha256(std::to_string(index) + hash_of_merkle_tree + previous_hash + timestamp + std::to_string(difficulty) + std::to_string(nonce));
	...
	set_nonce(nonce);
	set_duration(elapsed_seconds.count());
...
block::block(long long index, std::vector<tranx> transactions, std::string previous_hash, std::string timestamp, long long difficulty, long long nonce)
{
	this->index = index;
	this->hash = Hash_A_Block(index, transactions, previous_hash, timestamp, difficulty, nonce);
...
long long block::get_nonce() {
	return nonce;
}

long long block::get_difficulty() {
	return difficulty;
}

double block::get_duration() {
	return duration;
}

void block::set_nonce(long long n) {
	this->nonce = n;
}

void block::set_duration(double d) {
	this->duration = d;
}
